...
<%
    ...
    if (idDest > 0 && idProp > 0 && idDest != idMe && contenido != null && !contenido.trim().isEmpty()) {
        ...
        try {
            ...
            PreparedStatement psV = con.prepareStatement(
                ...
                "JOIN rol r ON r.id_rol=ur.id_rol WHERE u.id_usuario=? AND u.estado='Activa' AND r.nombre='agente'");
            psV.setInt(1, idDest);
            ...
            if (valido) {
                ...
                ps.executeUpdate();
                ps.close();
                registrarAuditoria(con, idMe, "Envió mensaje sobre la propiedad " + idProp + " al agente " + idDest);
                session.setAttribute("flashMsj", "Mensaje enviado correctamente.");
            } else {
                session.setAttribute("flashMsj", "Error: el destinatario no es un agente activo.");
            }
        } catch (Exception e) {
    ...
    response.sendRedirect(ctx + "/cliente/mensajes.jsp?con=" + idDest + "&prop=" + idProp);
%>